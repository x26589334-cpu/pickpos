...
- 바꿀 때는 **`_content/메뉴통일.ps1` 의 `$MENU` 를 고치고 스크립트를 돌린다.** 241개 페이지가 한 번에 맞춰진다.
  ...
  ```
- 경로는 위치에 따라 자동으로 붙는다(메인은 그대로, 루트의 다른 페이지는 `#`앞에 `./`, 하위 폴더는 `../`, 자기 폴더는 `./`). 루트의 모든 *.html 을 처리한다.
...
## 루트 안내 페이지 5개 — 네이버 색인용 (2026-09-16)
`pos.html` · `vending.html` · `kiosk.html` · `install.html` · `faq.html`
- **왜**: 같은 날 네이버에 등록한 형제 사이트는 색인 4~7건인데 픽포스만 1건이었다.
  비교해 보니 형제 사이트는 루트에 페이지가 여러 개(`/about.html` 등)였고, 색인된 것도 전부 루트 페이지였다.
  픽포스는 루트에 `index.html` 하나뿐이고 나머지가 전부 `compare/` `region/` `review/` 하위 폴더라 잡힐 페이지가 없었다.
  3개월 된 H포스는 하위 폴더도 색인되어 있다 → 네이버는 새 사이트에서 **루트 페이지를 먼저** 가져간다.
- **새 안내 페이지는 루트에 둔다.** 하위 폴더에 넣으면 초기 색인이 늦다.
- 상단 메뉴의 "포스기·카드단말기" → `pos.html`, "무인자판기" → `vending.html`. 메인 본문에서도 다섯 페이지로 링크한다.
- 메인 섹션 내용을 그대로 복사하지 않았다(중복 판정 방지). 각 페이지는 따로 늘려 쓴 글이다.
- `faq.html` 에는 FAQPage 구조화 데이터가 있다. 질문을 추가하면 JSON-LD 도 같이 고칠 것.
